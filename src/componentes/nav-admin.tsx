...
export function NavAdmin() {
  ...
  return (
    ...
    <nav className="-mx-6 flex items-center gap-1 overflow-x-auto border-b border-neutral-200 px-6 text-sm font-medium sm:mx-0 sm:px-0">
      {pestanas.map((p) => {
        // "/admin" es prefijo de todas las demas: sin el caso exacto, el Tablero se veria
        // activo en cualquier pestaña.
        const activa = p.href === "/admin" ? pathname === "/admin" : pathname.startsWith(p.href);
        return (
          <Link
            key={p.href}
            href={p.href}
