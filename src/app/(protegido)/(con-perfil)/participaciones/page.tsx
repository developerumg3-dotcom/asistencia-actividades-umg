...
export default async function ParticipacionesPage() {
  ...
  const tabla = await obtenerParticipaciones(alumnoActual.id);

  return (
    <main className="mx-auto flex min-h-dvh max-w-3xl flex-col gap-6 p-6">
      <NavAlumno />
      <div>
        <h1 className="text-xl font-semibold">Participaciones</h1>
        <p className="mt-1 text-sm text-neutral-600">
          Tus puntos por clase, actividad por actividad. Es la misma tabla que recibe tu
          catedrático.
        </p>
      </div>
